...
begin
  ...
  loop
    if not exists(
      select 1
      from jsonb_array_elements(
        coalesce((select settings->'murder_party'->'players'
                  from public.birthday_config where id=1),'[]'::jsonb)
      ) p
  ...
  -- Supprime uniquement les attributions retirées de la sélection.
